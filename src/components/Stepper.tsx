...
export default function Stepper({
  ...
  return (
    <div className="rb-stepper-outer" {...rest}>
      <div className={`rb-stepper-shell ${stepCircleContainerClassName}`}>
        <div className={`rb-stepper-indicator-row ${stepContainerClassName}`}>
          {stepsArray.map((_, index) => {
            const stepNumber = index + 1;
            const isNotLastStep = index < totalSteps - 1;
            return (
              <div className="rb-stepper-indicator-group" key={stepNumber}>
                {renderStepIndicator ? (
                  renderStepIndicator({
                    step: stepNumber,
                    currentStep,
        ...
        {!isCompleted && (
          <div className={`rb-stepper-footer ${footerClassName}`}>
            <div className={`rb-stepper-nav ${currentStep !== 1 ? "spread" : "end"}`}>
              {currentStep !== 1 && (
                <button onClick={handleBack} className="rb-stepper-back" {...backButtonProps}>
                  {backButtonText}
                </button>
              )}
              <button
                ...
                {...nextButtonProps}
              >
                {isCompleting ? "Saving..." : isLastStep ? completeButtonText : nextButtonText}
              </button>
            </div>
          </div>
        )}
...
function StepIndicator({
  ...
  return (
    ...
    >
      <motion.div
        variants={{
          inactive: { scale: 1, backgroundColor: "#222b38", color: "#9aa5b5" },
          active: { scale: 1, backgroundColor: "#d84a5f", color: "#d84a5f" },
          complete: { scale: 1, backgroundColor: "#00856f", color: "#fff" },
        }}
...
function StepConnector({ isComplete }: StepConnectorProps) {
  const lineVariants: Variants = {
    ...
    complete: { width: "100%", backgroundColor: "#00856f" },
  };
